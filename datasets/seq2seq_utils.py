...
from model_args import get_default_process_count
...
class Seq2SeqDataset(Dataset):
    def __init__(self, encoder_tokenizer, decoder_tokenizer, args, data, mode):
        args.process_count = get_default_process_count()
...
class SimpleSummarizationDataset(Dataset):
    """
    For BART, marian, blender, blender-large
    """
    ...
    def __init__(self, tokenizer, args, data, mode):
        ...
        if args.use_multiprocessing:  # 使用多进程
            logger.info(" Using multi process.")
            with Pool(args.process_count) as p:
                self.examples = list(
                    tqdm(
            ...
            logger.info(" Only use single process.")
    ...
    def __getitem__(self, index):
        # print("instance:", self.examples[index])
        return self.examples[index]
